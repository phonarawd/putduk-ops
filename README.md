# PUTDUK OPS · 퍼뜩 채굴 운영센터

이 레포는 퍼뜩의 **운영자 관제센터 프론트엔드**입니다. 특히 Mine OS 영역은 실제 Nest 관리자 API를 호출하며, 광산·수익률·운용·정산·서버 안전 제어가 화면의 중심입니다. 금융 권위는 백엔드와 Ledger에 있고, 이 레포는 Supabase에 직접 연결하지 않습니다.

## 꼭 지킬 구조

```text
PUTDUK_WEB  ──┐
              ├── 기존 Nest API: https://api.hiptk.app
PUTDUK_OPS  ──┘
```

- 이 레포에 백엔드 코드를 넣지 않습니다.
- 프론트에서 Supabase에 직접 연결하지 않습니다.
- 새 인증 방식, 새 DB, 새 서버를 만들지 않습니다.
- 운영자 인증·권한·감사 기록은 기존 백엔드 API가 최종 판단합니다.
- Mine OS 영역은 실제 관리자 API를 사용합니다. 연결되지 않은 기존 운영 기반 화면은 체험용 데이터가 실제 운영 데이터로 오해되지 않게 표시합니다.
...
- 퍼뜩 채굴 운영센터
- 광산 생성·수정·공개·중지·재개·종료
- 수익률 초안·승인·예약·적용
- 회원 운용 현황과 운용 상세
- 정산 조회·재실행
- 신규 운용 전체 중지·정산 보류 서버 제어
...
- 입금·출금·거래·금액 차이 확인
- 본인 확인
- 공지사항·이벤트·혜택·배너·알림 보내기
- 안전 관리
- 운영 현황
- 직원·승인 요청·작업 기록·열람 기록
- 서비스 상태·문제·점검·기능 켜기/끄기

전체 주소와 현재 연결 상태는 `ADMIN_SCREEN_MAP.md`, `ADMIN_INTEGRATION_MAP.md`, `ADMIN_BACKEND_GAP_REGISTER.md`를 확인합니다.

Cursor 인계 기준은 `CURSOR_HANDOFF.md`를 먼저 읽습니다.
