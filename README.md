# PUTDUK OPS · 퍼뜩 운영자 화면

이 레포는 퍼뜩 운영자 화면의 **프론트엔드 기준 소스**입니다.

현재 화면은 운영 API에 연결하기 전의 운영자 화면 프로토타입입니다. 화면 이동, 회원 상세, 퍼뜩AI 대화 열람 화면, 입출금 검토 화면, 공지·이벤트 화면, 상품 운영 화면을 먼저 완성해 두었고, Cursor AI가 기존 Nest 백엔드 API에 연결하는 작업을 이어서 진행합니다.
...
- 실제 API가 연결되기 전까지 화면의 체험용 데이터는 실제 운영 데이터로 오해되지 않게 표시해야 합니다.
